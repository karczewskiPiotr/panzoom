...
function createPanZoom(domElement, options) {
  ...
  var api = {
    ...
    smoothZoomExact: smoothZoomExact,
  ...
  /**
   * **BEWARE**: This function does not take bounds or min/max zoom into account. \
   * It basicallay applies the exact transform passed to the params.
   *
   * @param {number} clientX - x coordiante relative to transform origin
   * @param {number} clientY - y coordiante relative to transform origin
   * @param {number} scale - amount of zoom to apply
   */
  function zoomExact(clientX, clientY, scale) {
    if (isNaN(clientX) || isNaN(clientY) || isNaN(scale)) {
      throw new Error("zoom requires valid numbers");
    }
   ...
   * @param {number} clientX - x coordiante relative to transform origin
   * @param {number} clientY - y coordiante relative to transform origin
   * @param {number} toScaleValue - amount of zoom to apply
   */
  function smoothZoomExact(clientX, clientY, toScaleValue) {
    if (isNaN(clientX) || isNaN(clientY) || isNaN(toScaleValue)) {
      throw new Error("zoom requires valid numbers");
    }

    const fromScale = transform.scale;
    const fromX = transform.x;
    const fromY = transform.y;
    const from = { x: fromX, y: fromY, scale: fromScale };
    const to = { x: clientX, y: clientY, scale: toScaleValue };
